...
const stormIconDisplayEl = document.getElementById("stormIcon");
  ...
  function activateStorm() {
    ...
    if (powerMode === "warshark") {
      ...
      // Initial 4 sharks (~25% fewer): 2 forward arcs, 2 outer sweeps
      var px0 = player.x, py0 = player.y;
      var initSharks = [
        { mode: "circle", hold: 0.15, ang: -0.2, circleT: 0.35 },
        { mode: "circle", hold: 0.15, ang: 0.2, circleT: 0.35 },
        { mode: "circle", hold: 0.28, ang: -1.15, circleT: 0.55 },
        { mode: "circle", hold: 0.28, ang: 1.15, circleT: 0.55 }
      ];
...
if (window.__airbornePlasmaIgnite) {
    ...
    if (stormMode === "heatseek" || stormMode === "warshark" || stormMode === "barrelbomb" || stormMode === "jollybomb") {
      ...
      if (!untilHs || nowHs >= untilHs) {
        ...
        window.__airborneActivePowerVisual = null;
        window.__airborneActivePowerUntil = 0;
        // Kill predator sonar/reticles immediately when power window ends
        if (window.__airbornePredator) {
          window.__airbornePredator.sonarPulses = [];
          window.__airbornePredator.reticles = {};
          window.__airbornePredator.sonarT = 999;
...
// Bullets
  ...
  // Ivory Anchor orb asset (steampunk ivory/brass, bg removed)
  window.__airborneIvoryAnchorImg = window.__airborneIvoryAnchorImg || (function() {
    var im = new Image();
    im.src = "ivory_anchor_orb.png?v=ruff291";
    return im;
  })();
  ...
  function drawRoyalCoronation() {
    ...
    if (alive.length >= 2) {
      ...
      for (var ci = 0; ci < alive.length; ci++) {
        ...
        var ax1 = a1.chainX != null ? a1.chainX : a1.x;
